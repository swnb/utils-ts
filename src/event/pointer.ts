...
export function useSelectedPointermove({
  ...
  onMultiTouchmove?: (pointer1: TypeArray<Position, 2>, pointer2: TypeArray<Position, 2>) => void
  ...
  useEffect(() => {
    const callback = (ev: TouchEvent) => {
      ...
      if (touchLength == 1) {
        ...
        if (previousMultiPos) {
          onMultiTouchmoveCache([previousMultiPos[0], pointer1], [previousMultiPos[1], pointer2])
